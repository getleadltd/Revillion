...
const TOPIC_VISUALS: Array<{ pattern: RegExp; subjects: string[] }> = [
  ...
  {
    pattern:
      /scommesse?|sportsbook|sport betting|calcio|football|match|partita|quote/i,
    subjects: [
  ...
  {
    pattern:
      /affiliat|commission|CPA|revshare|partner|referral|network|guadagn/i,
    subjects: [
  ...
  {
    pattern:
      /SEO|traffico|traffic|keyword|posizionam|google|ranking|ricerca organic/i,
    subjects: [
  ...
  {
    pattern:
      /bonus|promozione|offerta|welcome|free spins|giri gratis|cashback|no deposit/i,
    subjects: [
  ...
  {
    pattern:
      /responsabile|sicurezza|protezione|limite|dipendenza|autoesclusione/i,
    subjects: [
  ...
  {
    pattern:
      /pagamento|payment|withdrawal|prelievo|deposito|bonifico|carta|Skrill|Neteller/i,
    subjects: [
  ...
  {
    pattern:
      /torneo|tournament|competizione|campionato|leaderboard|classifica/i,
    subjects: [
  ...
  {
    pattern:
      /come diventare|getting started|iniziare|primo passo|beginner|principiante|guida per|how to become/i,
    subjects: [
  ...
  {
    pattern:
      /guadagn|earn|income|revenue|stipendio|entrate|soldi|money|profit|ricavo/i,
    subjects: [
  ...
  {
    pattern:
      /traffico|traffic source|telegram|social media|youtube|tiktok|instagram|SEO|PPC|email marketing/i,
    subjects: [
...
const VISUAL_STYLES = [
  {
    ...
    composition:
      "rule of thirds, shallow depth of field f/1.8, bokeh background",
  },
  {
    ...
    composition:
      "centered composition with generous negative space, minimalist",
  },
  {
    style: "cinematic photography in style of Hollywood production still",
    lighting:
      "motivated lighting from practical sources, lens flare, anamorphic",
    mood: "tense, atmospheric, immersive, filmic",
    composition:
      "wide angle establishing shot, dynamic angle, foreground elements",
  },
  {
  ...
  },
  {
    ...
    composition:
      "extreme close-up, filling the frame, perfect focus on texture",
  },
  {
...
function buildVisualConcept(title: string, category: string): string {
  ...
  const titleHash = title.split("").reduce(
  ...
  const style = pickRandom(
    VISUAL_STYLES,
    (titleHash * 7 + 13) % VISUAL_STYLES.length,
  );
  ...
  try {
    ...
    if (autoPrompt?.title) {
      const visualConcept = buildVisualConcept(
        autoPrompt.title,
        autoPrompt.category || "news",
      );

      finalPrompt =
        `Create a professional photograph for a blog article titled: "${autoPrompt.title}".
...
REMINDER: ABSOLUTELY ZERO text, letters, numbers, or symbols visible anywhere in the image.`;
    ...
    if (!finalPrompt?.trim()) {
      return new Response(JSON.stringify({ error: "Prompt è obbligatorio" }), {
        status: 400,
    ...
    if (!aiResponse.ok) {
      ...
      if (aiResponse.status === 429) {
        return new Response(
          JSON.stringify({ error: "Rate limit raggiunto." }),
          {
            status: 429,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          },
    ...
    return new Response(
      JSON.stringify({
        error: msg || "Errore durante la generazione immagine",
      }),
      {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
