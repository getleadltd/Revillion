...
serve(async (req) => {
  ...
  try {
    ...
    if (!topic?.trim()) {
      return new Response(JSON.stringify({ error: "Topic è obbligatorio" }), {
        status: 400,
    ...
    const { data: clusterArticles } = await supabase
      .from("blog_posts").select(
        "slug_it, slug_en, title_it, title_en, category, views",
      )
    ...
    const toneGuide = ({
      ...
      casual: "diretto e amichevole",
      technical: "tecnico e preciso",
    } as any)[tone || "professional"];
    const formatGuide = ({
      listicle: "Lista numerata con H2 per ogni punto",
      howto: "Step-by-step con <ol>",
      review: "Recensione con <blockquote> per il verdetto",
      comparison: "Confronto con tabella HTML",
      news: "Lead paragraph + dettagli",
    } as any)[content_format || "howto"] || "Struttura con H2 logici";
    const intentGuide = ({
      informational: "Definizione chiara + esempi pratici",
      transactional: "Benefici concreti + passi immediati",
      commercial: "Tabella comparativa + raccomandazione",
      navigational: "Diretto, senza intro prolissa",
    } as any)[search_intent || "informational"];

    const siloContext = [
      pillarArticle
        ? `PILLAR: "${
          pillarArticle.title_it || pillarArticle.title_en
        }" → /blog/${pillarArticle.slug_it || pillarArticle.slug_en}`
        : "",
      clusterLinks.length > 0
        ? "CLUSTER:\n" +
          clusterLinks.slice(0, 5).map((a: any) =>
            `- "${a.title_it || a.title_en}" → /blog/${a.slug_it || a.slug_en}`
          ).join("\n")
        : "",
      crossCluster.length > 0
        ? "CROSS-CLUSTER:\n" +
          crossCluster.slice(0, 3).map((a: any) =>
            `- [${a.category}] "${a.title_it || a.title_en}" → /blog/${
              a.slug_it || a.slug_en
            }`
          ).join("\n")
        : "",
    ...
    const contentPrompt =
      `Sei un esperto di affiliate marketing iGaming. Scrivi un articolo SEO per affiliati casino su: "${topic}"
...
${
        siloContext ||
        "Nessun articolo pubblicato ancora — scrivi senza link interni."
      }
...
HTML puro: <h2>, <h3>, <p>, <ul>/<ol>, <table>, <strong>. NO html/body/article wrapper. Inizia direttamente con il contenuto.`;
    ...
    const contentRaw = await callAI(LOVABLE_API_KEY, [{
      role: "user",
    ...
    if (!contentRaw || contentRaw.length < 500) {
      throw new Error(
        `Contenuto generato troppo corto: ${contentRaw.length} caratteri`,
      );
    }

    // Strip markdown code fences if model added them
    const content_it = contentRaw.replace(/^```(?:html)?\s*/im, "").replace(
      /\s*```\s*$/im,
      "",
    ).trim();
    ...
    const metaPrompt =
      `Dato questo articolo su "${topic}" per affiliati iGaming, genera i metadati SEO.
...
Usa 4-6 FAQ rilevanti per il topic. Schema può essere: Article, HowTo, Review, FAQPage.`;
    ...
    const metaRaw = await callAI(LOVABLE_API_KEY, [{
      role: "user",
      content: metaPrompt,
    }], 1024);
    ...
    const metaClean = metaRaw.replace(/^```(?:json)?\s*/im, "").replace(
      /\s*```\s*$/im,
      "",
    ).trim();
    ...
    try {
      ...
      console.warn(
        "Metadata JSON parse failed, attempting field extraction...",
      );
      const extract = (key: string) => {
        const m = metaJsonMatch[0].match(
          new RegExp(`"${key}"\\s*:\\s*"((?:[^"\\\\]|\\\\.)*)"`),
        );
      ...
      if (kwMatch) {
        meta.keywords = kwMatch[1].match(/"([^"]+)"/g)?.map((s: string) =>
          s.replace(/"/g, "")
        ) ?? [];
      }
    ...
    const slugify = (t: string) =>
      t.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(
        0,
        55,
      );
    const faq_items = (meta.faq || []).map((f: any) => ({
      question: f.q || f.question,
      answer: f.a || f.answer,
    }));
    const wordCount = content_it.split(/\s+/).filter(Boolean).length;
    const internalLinks =
      (content_it.match(/<a\s+href=["']\/blog\/[^"']+["']/gi) || []).length;
    console.log(
      `✅ Words: ${wordCount}, Silo links: ${internalLinks}, FAQ: ${faq_items.length}`,
    );
